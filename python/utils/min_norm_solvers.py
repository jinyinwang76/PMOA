...
class MGDASolver:
    ...
    def find_min_norm_element(vecs: list):
        ...
        给定一系列向量(vecs)，该方法找到凸包中的最小范数元素为 min |u|_2 st. u = \sum c_i vecs[i] 和 \sum c_i = 1。
        相当于是几何的，其主要思想是：如果 d_{ij} = min |u|_2 st u = c x_i + (1-c) x_j；
        解位于（0，d_{i，j}）。因此，我们找到最佳的2任务解，然后运行投影梯度下降直到收敛。
        """
